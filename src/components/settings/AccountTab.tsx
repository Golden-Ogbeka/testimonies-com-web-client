...
export default function AccountTab() {
  ...
  const onEmailSubmit = async (v: { email: string }) => {
    try {
      await updateEmail.mutateAsync(v);
      setOtpSent(true);
  ...
  const onVerifyOtp = async () => {
    try {
      await verifyEmailOtp.mutateAsync(otpCode);
  ...
  const onUsernameSubmit = async (v: { username: string }) => {
    try {
      await updateUsername.mutateAsync(v);
  ...
  const onPhoneSubmit = async (v: { phoneNumber: string }) => {
    try {
      await updatePhone.mutateAsync(v);
  ...
  const onPasswordSubmit = async (v: { oldPassword: string; newPassword: string; confirmNewPassword: string }) => {
    try {
      await updatePassword.mutateAsync(v);
  ...
  return (
    <>
      ...
      <div className="rounded-none border border-border bg-background p-4">
        <h2 className="mb-4 text-sm font-bold text-foreground">Password</h2>
